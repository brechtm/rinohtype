...
from pyte.backend.pdf import cos
...
class PDF(object):
    ...
    def next_token(self):
        ...
        if token in (HEXSTRING_BEGIN, HEXSTRING_END):
    ...
    def next_item(self):
        ...
        if token == STRING_BEGIN:
            item = self.read_string()
        elif token == HEXSTRING_BEGIN:
            item = self.read_hex_string()
        elif token == ARRAY_BEGIN:
            item = self.read_array()
        elif token == NAME_BEGIN:
            item = self.read_name()
        elif token == DICT_BEGIN:
...
if __name__ == '__main__':
    pdf = PDF('../examples/rfic2009/fig2.pdf')
