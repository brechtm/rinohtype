...
from .text import StyledText, MixedStyledText, MixedStyledTextBase, TextStyle
...
class GlossaryDefinitionStyle(TextStyle):
    replace = OverrideDefault(StyledText.from_string("' ({SELF})'"))


class GlossaryDefinition(MixedStyledText):
    style_class = GlossaryDefinitionStyle
...
class GlossaryTerm(MixedStyledTextBase):
    ...
    def children(self, container, replace_self=None):
        ...
        try:
            ...
            return
        definition_text = GlossaryDefinition(definition, parent=self)
